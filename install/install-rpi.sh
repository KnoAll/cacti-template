...
func_dbask () {
	  ...
	  read -n 1 -p "Enter 1 to use an untouched Cacti DB or 2 to use Kevin's tweaked DB: " db
		...
		curl -s https://raw.githubusercontent.com/KnoAll/cacti-template/rpi-template/install/mysql.cacti_clean.sql | sudo mysql cacti
		if [ $? -ne 0 ];then
			echo -e "\033[31m Something went wrong importing Cacti database, exiting..."
			echo -e -n "\033[0m"
		exit 1
...
if [ $? -ne 0 ];then
	...
	sudo sed  -i '$ a innodb_buffer_pool_size=226M' /etc/mysql/my.cnf
