...
upgradePHP() {
	...
	sudo yum install -y -q http://rpms.remirepo.net/enterprise/$remi && sudo yum install -y -q yum-utils &&	printinfo "Enabling new $php_description" && sudo yum-config-manager --enable remi-$php_version && sudo yum -y -q update
	if [ $? -ne 0 ];then
